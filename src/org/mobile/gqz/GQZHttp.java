...
import org.mobile.gqz.http.HttpCache;
import org.mobile.gqz.http.HttpHandler;
import org.mobile.gqz.http.RequestParams;
import org.mobile.gqz.http.ResponseStream;
import org.mobile.gqz.http.SyncHttpHandler;
import org.mobile.gqz.http.callback.HttpRedirectHandler;
import org.mobile.gqz.http.callback.RequestCallBack;
import org.mobile.gqz.http.client.DefaultSSLSocketFactory;
import org.mobile.gqz.http.client.HttpRequest;
import org.mobile.gqz.http.client.RetryHandler;
import org.mobile.gqz.http.entity.GZipDecompressingEntity;
import org.mobile.gqz.task.PriorityExecutor;
import org.mobile.gqz.utils.OtherUtils;

import android.text.TextUtils;

public class GQZHttp {
	...
	public GQZHttp() {
		this(GQZHttp.DEFAULT_CONN_TIMEOUT, null);
	}

	public GQZHttp(int connTimeout) {
		this(connTimeout, null);
	}

	public GQZHttp(String userAgent) {
		this(GQZHttp.DEFAULT_CONN_TIMEOUT, userAgent);
	}

	public GQZHttp(int connTimeout, String userAgent) {
	...
	public GQZHttp configResponseTextCharset(String charSet) {
	...
	public GQZHttp configHttpRedirectHandler(
			HttpRedirectHandler httpRedirectHandler) {
		this.httpRedirectHandler = httpRedirectHandler;
		return this;
	}

	public GQZHttp configHttpCacheSize(int httpCacheSize) {
		sHttpCache.setCacheSize(httpCacheSize);
		return this;
	}

	public GQZHttp configDefaultHttpCacheExpiry(long defaultExpiry) {
		HttpCache.setDefaultExpiryTime(defaultExpiry);
		currentRequestExpiry = HttpCache.getDefaultExpiryTime();
		return this;
	}

	public GQZHttp configCurrentHttpCacheExpiry(long currRequestExpiry) {
		this.currentRequestExpiry = currRequestExpiry;
		return this;
	}

	public GQZHttp configCookieStore(CookieStore cookieStore) {
		httpContext.setAttribute(ClientContext.COOKIE_STORE, cookieStore);
		return this;
	}

	public GQZHttp configUserAgent(String userAgent) {
		HttpProtocolParams.setUserAgent(this.httpClient.getParams(), userAgent);
		return this;
	}

	public GQZHttp configTimeout(int timeout) {
	...
	public GQZHttp configSoTimeout(int timeout) {
		final HttpParams httpParams = this.httpClient.getParams();
		HttpConnectionParams.setSoTimeout(httpParams, timeout);
		return this;
	}

	public GQZHttp configRegisterScheme(Scheme scheme) {
		this.httpClient.getConnectionManager().getSchemeRegistry()
				.register(scheme);
		return this;
	}

	public GQZHttp configSSLSocketFactory(SSLSocketFactory sslSocketFactory) {
	...
	public GQZHttp configRequestRetryCount(int count) {
		this.httpClient.setHttpRequestRetryHandler(new RetryHandler(count));
		return this;
	}

	public GQZHttp configRequestThreadPoolSize(int threadPoolSize) {
		GQZHttp.EXECUTOR.setPoolSize(threadPoolSize);
		return this;
	}
