...
public class Table {
    ...
    private Table(GQZDb db, Class<?> entityType) {
        this.db = db;
    ...
    public static synchronized Table get(GQZDb db, Class<?> entityType) {
    ...
    public static synchronized void remove(GQZDb db, Class<?> entityType) {
        String tableKey = db.getDaoConfig().getDbName() + "#" + entityType.getName();
        tableMap.remove(tableKey);
    }

    public static synchronized void remove(GQZDb db, String tableName) {
        if (tableMap.size() > 0) {
            String key = null;
