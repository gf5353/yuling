...
public class AsyncDrawable<T extends View> extends Drawable {
    ...
    private final WeakReference<GQZBitmap.BitmapLoadTask<T>> bitmapLoadTaskReference;

    private final Drawable baseDrawable;

    public AsyncDrawable(Drawable drawable, GQZBitmap.BitmapLoadTask<T> bitmapWorkerTask) {
        if (bitmapWorkerTask == null) {
            throw new IllegalArgumentException("bitmapWorkerTask may not be null");
        }
        baseDrawable = drawable;
        bitmapLoadTaskReference = new WeakReference<GQZBitmap.BitmapLoadTask<T>>(bitmapWorkerTask);
    }

    public GQZBitmap.BitmapLoadTask<T> getBitmapWorkerTask() {
