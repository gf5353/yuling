...
public class TimeUtil {
	...
	public final static String FORMAT_DATE_ALL = "yyyyMMddHHmmss";
	...
	public static boolean isYesterday(String strTime) {
		if (strTime.length() == 0) {
			return false;
		}
		String strTmp = strTime;
		if (strTime.length() >= 10) {
			strTmp = strTime.substring(0, 10);
		}
		String strCurrentTime = TimeToString(getDayHour(new Date(), -1));
		if (strTmp.equals(strCurrentTime)) {
			// 相等 是今天
			return true;
		} else {
			// 不等 不是今天
	...
	public static String TimeToString(Date dTime) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		return sdf.format(dTime);
	}

	// 获取iNum小时以后的时间
	public static Date getDayHour(Date date, int iNum) {
